...
async def bridge(reader:asyncio.StreamReader, writer:asyncio.StreamWriter, tmux_name:str, detach_first:bool, term_type:str):
    pid, master_fd = pty.fork()
    if pid==0:
        term = (term_type or "").strip() or os.environ.get("TERM", "")
        if term.lower() == "dumb" or not term:
            term = "vt100"
...
async def handle_client(reader, writer):
    ...
    try:
        ...
        term_type = env.get("TERM_TYPE", "")
        await bridge(reader, writer, tmux_name, detach_first, term_type)
